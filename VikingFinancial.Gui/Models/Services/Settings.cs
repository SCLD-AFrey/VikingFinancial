...
public class Settings
{
    ...
    public async Task WriteSettings()
    {
        try
        {
            await using var fs = File.Create(m_requiredFilesService!.SettingsPath);
    ...
    {
        if (!File.Exists(m_requiredFilesService!.SettingsPath))
        {
            await WriteSettings();
        }

        await using var storedSettings = File.OpenRead(m_requiredFilesService.SettingsPath);
